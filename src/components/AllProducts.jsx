...
function AllProduct() {
    ...
    const loadDataFromServer = (page = 0, pagesize = 9) => {
    ...
    const addToCart = item => {
        if (sessionStorage.getItem("userid") == null) {
            ...
            notifyError("Please login first to buy product")
            history.push("/clogin")
        }
        else if (sessionStorage.getItem("role") !== "customer") {
            notifyError("Only customer can buy product")
        }
        else {
            if (checkItem(item.prodid)) {
                ...
                notifyError("Item already in cart")
            }
        }
    }
